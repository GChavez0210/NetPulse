...
A new Console tab — interactive terminal access to network devices over SSH, Telnet, and a serial console cable — and exports across the app that report where they wrote.
...
- **Multiple concurrent sessions** (up to 16) as renameable tabs, each with its own terminal and scrollback. Sessions stay live while you work in other NetPulse tabs. The session bar reads `2 live · 2 tabs`, naming the cap only as you approach it; the Connect button reads *Session limit reached* at the point it applies. Selecting a tab focuses its terminal.
- **Terminal tools:** search across scrollback, clear, and export the session transcript to a text file. There is no Copy button — NetPulse registers no context menu of its own, so the WebView's native copy is what you already have.
- **A live tab warns before closing.** Closing a tab disconnects the session and discards its scrollback, so the confirmation names the transport, says the output goes too, and offers to export the transcript first. A tab that is already disconnected closes without ceremony. Escape cancels.
...
- **`Press R to reconnect`.** A session closed by anything other than you — an idle timeout, a reboot, a dropped link, a remote `exit` — prints the hint under the reason it closed, and R brings it back without retyping credentials. A device closing an idle session sends a clean end-of-stream, byte for byte what a typed `exit` sends, so SSH offers nothing to tell the two apart and both are treated as recoverable; only a disconnect you asked for, a closed tab, or leaving the app ends a session for good. R is claimed ahead of xterm's `disableStdin` gate, which is what makes it safe to take: a session in that state has no transport to type into.
- **Exports report the path they wrote to.** Console transcripts, ping session CSVs, flood CSVs, traceroute CSVs, and WHOIS TXT files are written by the backend into the downloads folder, and the absolute path appears in that tab's status toast. Handing a download to the WebView, as every export used to, puts the file somewhere the app cannot name. Export file names are built from hostnames and renameable tab labels, so the backend keeps only the final path component and replaces anything outside `[A-Za-z0-9._-]`; a name collision takes a numeric suffix rather than overwriting an existing capture.

### Changed
- **The Ping tab's CSV export reported nothing at all** — no success message, no failure message. It now reports like every other export, including the target count.
- **The SSL / TLS Inspector's port is a plain text box** rather than a number input with stepper arrows, matching the console's TCP port field — a port is typed, not stepped. It accepts digits only, up to five, with the 1–65535 range checked when the tool runs. It also stops snapping back to 443 mid-edit: the value was re-parsed against a fallback on every keystroke, so clearing the field to type a new port refilled it with 443.
...
- SSH credentials are kept outside React component state and are discarded as soon as a session can no longer need them — on a deliberate disconnect, a closed tab, or leaving the app. A session closed by the remote end retains them, so that pressing R does not require retyping; the trade is that a password survives an idle timeout in memory for as long as that tab stays open.
